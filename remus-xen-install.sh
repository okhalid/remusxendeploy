...
if [ "$MODE" == "1" ]; then
	...
	remote=true
	local=false
	echo "Will download latest and greatest Xen and Kernal sources, and will build from it."
	echo "Stay put; it's going to take some time."
fi
...
install_ocaml()
{
	if [ "$HOSTTYPE" == "i386" ]; then
		...
		rpm -ivh ocaml-3.12.1-13.1.i586.rpm
        	...
        	echo $HOSTTYPE
	        wget http://download.opensuse.org/repositories/devel:/languages:/ocaml/openSUSE_11.4/x86_64/ocaml-3.12.1-13.1.x86_64.rpm
...
echo "Initialize and push persisten_bitmap"
...
install_linux_kernel()
{
	...
	#Downloading the two configurationg files is disabled. Configurations files are shipped with this script.
	...
	if [ ! -e "$local_kernel_config" ]; then
		echo "Xen kernal configuration file doesn't exist, downloading..."
